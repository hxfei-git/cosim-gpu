...
本仓库也是用户从 BSP 工程师转型为 GPU 驱动工程师的实践学习仓库。用户已有
MMU、cache、ARM、中断和一定的 QEMU 基础；解释和规划应优先建立这些知识与
GPUVM、queue/doorbell、dispatch、completion 和驱动故障边界之间的联系。

- [`docs/学习路线.md`](docs/学习路线.md) 是学习目标、阶段边界、当前状态和调整记录
  的唯一规划入口。处理学习规划、阶段实施、进度复盘或学习实验前必须先读取该文件，
  遵守“每阶段完成后暂停、由用户决定是否进入下一阶段”的约束。
- 用户还在
  `/mnt/c/Users/28431/Desktop/GPU学习/01_GPU系统基础` 学习 GPU 系统基础。每次制定
  阶段计划或复盘时，只读取与当前主题直接相关的材料，并与本仓库 artifact 和固定
  版本源码相互核对。
- 用户明确反馈新的阅读进度、自检答案、疑问或实验结果后，应同步更新
  `docs/学习路线.md` 的“当前学习状态”与必要的“路线调整记录”，不能只把进度保留
  在对话中，也不能仅根据文件存在推断用户已经掌握。
- 外部材料中的通用 HSA/ROCm 概念可以迁移；RDNA 3/RX 7800 XT 的 WGP/CU 组织、
  wave 模式、Firmware 路径和硬件数量不能直接作为当前 `gfx942`/MI300X cosim 的
  事实，必须标注证据边界。
- 外部仓库默认只读。除非用户明确要求，不修改、提交或清理其中的文件；本仓库中的
  学习路线可以根据已确认进展持续更新，但是否进入下一 Phase 仍由用户决定。
...
- 代理生成的说明、文档、注释和用户交互使用中文；命令、路径、代码标识符、协议字段
  与工具原始输出可以保留原文。
- 根目录只保留 `README.zh.md`；当前正式入口直接放在单层 `docs/` 下，使用中文
  文件名和中文内容，不维护重复的英文副本。
- `docs/文档索引.md` 是统一文档入口，`docs/学习路线.md` 是当前学习规划基准。
  `docs/tmp/` 仅作为旧资料备份，不是运行临时目录，也不属于当前正式文档；不要在
  README、文档索引或正式文档中链接它，不要让文档合同扫描它。除非用户明确要求，
  不读取、修改、删除、清空或迁移其中内容。
